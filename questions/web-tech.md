...
## **Что из себя представляет JWT**

**JWT (JSON Web Token)** — это стандарт для создания токенов, которые содержат закодированные данные. Состоит из трех частей:

1. **Header:**  
   Содержит информацию о типе токена и алгоритме шифрования.

2. **Payload:**  
   Содержит данные (например, ID пользователя, роли).

3. **Signature:**  
   Подпись, которая гарантирует целостность токена.

**Использование:**  
JWT применяется для аутентификации и передачи данных между клиентом и сервером.
...
## 10. Этапы рендеринга в браузере

### 1. **Парсинг HTML → Создание DOM**
Браузер анализирует HTML-документ и строит **DOM (Document Object Model)** — структуру, представляющую HTML-элементы как объекты.

### 2. **Парсинг CSS → Создание CSSOM**
Параллельно с HTML, браузер загружает и парсит CSS, создавая **CSSOM (CSS Object Model)**, который представляет стили элементов.

### 3. **Style Calculation (Вычисление стилей)**
На этом этапе браузер вычисляет, какие стили применяются к каждому элементу, какие наследуются, а какие переопределяются. Вычисляются активные **медиа-запросы**.

### 4. **Создание Render Tree**
Браузер объединяет **DOM** и **CSSOM** в **Render Tree**, исключая невидимые элементы (например, с `display: none`). Этот этап включает все видимые элементы с их вычисленными стилями.

### 5. **Layout (Вычисление позиций и размеров)**
Браузер рассчитывает **размеры** и **положение** каждого элемента на странице. Эта операция включает работу с такими свойствами, как `width`, `height`, `margin`, `padding`, и другие.

### 6. **Paint (Рисование пикселей)**
После расчёта макета браузер рисует пиксели, добавляя цвета, фоны, градиенты, тени и другие визуальные эффекты для каждого элемента.

### 7. **Composite (Сборка слоёв и отрисовка)**
Браузер объединяет все элементы на экране в один кадр, создавая **составной слой**. Это позволяет эффективно отрисовывать страницы, используя отдельные слои для анимаций, прокрутки и прозрачности.

---

## Как ускорить рендеринг?
- Использовать **критический CSS** (выносить важные стили inline).
- Минимизировать **пересчёты Layout** (избегать изменений `width`, `height`, `margin`).
- Применять **`transform` и `opacity`** для анимаций, чтобы не пересчитывать Layout.
- Минимизировать использование **глубоких вложенных селекторов** и **сложных CSS-селекторов**.
- Использовать **классы**, а не вложенные селекторы, для оптимизации.

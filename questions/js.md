...
## 81. **Минусы `JSON.stringify()`**

`JSON.stringify()` — это метод в JavaScript, который преобразует объект в строку JSON. Однако у него есть несколько недостатков:

### **Потеря данных**  
Игнорирует функции, `undefined`, `Symbol`, `Infinity`, `NaN`.

### **Циклические ссылки**  
Вызывает ошибки при наличии циклических ссылок в объекте.

### **Производительность**  
Может быть медленным при работе с большими объектами.

### **Форматирование**  
По умолчанию не поддерживает красивое форматирование (нужны дополнительные параметры).

### **Типы данных**  
Преобразует все значения в строки, что может привести к потере точности (например, с `BigInt`).

## 82. **Что такое DOM и BOM?**
...
## 84. **Что такое распространение события (Event Propagation)?**
...
## 85. **Разница между e.preventDefault() и e.stopPropagation(), .stopImmediatePropagation()?**
...
## 86. **Как узнать об использовании метода event.preventDefault()?**
...
## 87. **Что такое делегирование событий (Event Delegation)?**
...
## 90. **Методы поиска элементов в DOM?**
...
## 91. **Как добавить обработчик события на DOM-элемент?**
...
## 92. **Как удалить обработчик события с DOM-элемента?**
...
## 93. **Свойства для перемещения по DOM-дереву?**
...
## 94. **Разница между attribute и property у DOM-элементов?**
...
## 96. **Разница между событиями load и DOMContentLoaded?**
...
## 97. **Разница между HTMLCollection и NodeList?**
...
## 98. **Как динамически добавить элемент на HTML-страницу?**
...
## 99. **Разница между feature detection, feature inference и анализом строки user-agent?**
...
## 100. **Сколько аргументов принимает addEventListener?**
...
## 101. **Разница между innerHTML и outerHTML?**
...
## 102. **Для чего используется метод .focus()?**
...
## 103. **Для чего используется свойство .forms?**

Свойство `.forms` объекта `document` возвращает коллекцию всех форм в документе.

## 104. **Для чего используется метод .scrollIntoView()?**
...
## 105. **Разница между методами .submit() и .requestSubmit()?**
...
## 106. **Расскажите о URLSearchParams?**
...
## 107. **Как можно создавать пользовательское событие (custom events) в JavaScript?**
...
## 108. **Расскажите о методе requestAnimationFrame()?**

[LearnJS](https://learn.javascript.ru/js-animation)

## 109. **Для чего используется свойство .dataset?**
...
## **109. Что такое идемпотентность?**

### **Определение**
Идемпотентность — это свойство операции, при котором повторное выполнение операции дает тот же результат, что и первое выполнение.

### **Где встречается?**
- **HTTP-запросы:** Некоторые методы запросов являются идемпотентными.
- **Базы данных:** Операции, которые не изменяют состояние при повторном выполнении.
- **API:** Обеспечение безопасности и предсказуемости при повторных запросах.

### **Идемпотентные методы HTTP**
- **GET:** Получение данных. Повторные запросы не изменяют состояние.
- **PUT:** Обновление ресурса. Повторные запросы с теми же данными не изменяют результат.
- **DELETE:** Удаление ресурса. Повторные запросы не изменяют состояние (ресурс уже удален).

### **Неидемпотентные методы HTTP**
- **POST:** Создание ресурса. Повторные запросы могут создавать новые ресурсы.
- **PATCH:** Частичное обновление ресурса. Результат может зависеть от текущего состояния.
- 
### 110. **Что это?**
- **Service Worker:** Скрипт, который работает в фоновом режиме, отдельно от веб-страницы.
...
### **Как работает?**
1. **Регистрация:** Service Worker регистрируется на странице.
2. **Установка:** Кеширует необходимые ресурсы.
3. **Активация:** Управляет кешем и перехватывает сетевые запросы.
4. **Фетч:** Перехватывает запросы и решает, как их обработать (например, из кеша или сети).

   ## **DRY (Don't Repeat Yourself)**

- **Цель:** Избегать дублирования кода.
- **Пример:**
  ```javascript
  // Плохо: Дублирование
  function area(r) { return 3.14 * r * r; }
  function circumference(r) { return 2 * 3.14 * r; }

  // Хорошо: Нет дублирования
  const PI = 3.14;
...
- **Цель:** Делать код простым и понятным.
- **Пример:**
  ```javascript
  // Плохо: Сложно
...
### **Что это?**
- **XSS:** Уязвимость, при которой злоумышленник внедряет вредоносный код (обычно JavaScript) на страницу, которую видят другие пользователи.

### **Типы XSS:**
1. **Хранимый (Stored XSS):**
   - Вредоносный код сохраняется на сервере (например, в базе данных) и выполняется при каждом открытии страницы.
2. **Отраженный (Reflected XSS):**
   - Вредоносный код передается через URL или параметры запроса и сразу выполняется в браузере жертвы.
3. **DOM-based XSS:**
   - Вредоносный код внедряется через манипуляции с DOM, без отправки данных на сервер.
